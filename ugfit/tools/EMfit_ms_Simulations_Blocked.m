function [modout]= EMfit_ms_Simulations_Blocked(rootfile,modelID,quickfit)
...
fit.ntrials = nan(length(rootfile.beh.offer),1);
for is = 1:n_subj
   fit.ntrials(is) = sum(~isnan(rootfile.beh.offer{is}),'All' ); 
end
...
for is = 1:n_subj
   ...
   modout.(modelID).sub{is}.norm  = subfit.norm;
   modout.(modelID).sub{is}.V  = subfit.V;
