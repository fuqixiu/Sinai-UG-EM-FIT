function [ out ] = EMmc_ms_Hess_fix(rootfile,modnames)
...
for imod=1:numel(modnames)
     ...
     % Miriam/Pat Jan 2020 - from Sam Gershman, use BIC if any bad Hessians
%     % ================================== %
     %if any(rootfile.em.(modnames{imod}).fit.goodHessian==0)
         lme0 = -0.5*rootfile.em.(modnames{imod}).fit.bic; %bic
         lme(:,imod) = lme0;
         bicint(imod)= sum(rootfile.em.(modnames{imod}).fit.bic);
     %else
%     % ==================================== %
...
compM   = {'ms_UG0_f0f_adaptiveNorm','ms_UG2_etaf_f0f_adaptiveNorm'};                                    % models to compare specifically
subplot(2,2,3);
for imod=1:numel(compM)
   lmepair(:,imod)    = -0.5*rootfile.em.(modnames{imod}).fit.bic;%rootfile.em.(compM{imod}).fit.lme;
