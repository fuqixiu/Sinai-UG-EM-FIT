function [fval,fit] = mod_ms_UG0_Bayes_10(behavData,q, doprior,dofit,varargin)
...
[qt, bounds] = norm2par('ms_UG0_Bayes_10',q); % transform parameters from gaussian space to model space
...
fixed =[10, 4]; % Fixed f0, k paramter
free = {alpha beta};
% -------------------------------------------------------------------------------------
% 2-4) Middle code is specific the the model
% -------------------------------------------------------------------------------------
if doprior == 1
    [fval,norm,V,ChoiceProb] = lik_UG0_Bayes_10(behavData.offer, behavData.choice,fixed,free,doprior,prior,q);
else
    [fval,norm,V,ChoiceProb] = lik_UG0_Bayes_10(behavData.offer, behavData.choice,fixed,free,doprior);
