function [fval,fit] = mod_ms_UG1_etaf_fixedNorm(behavData,q, doprior,dofit,varargin)
...
[qt, bounds] = norm2par('ms_UG1_etaf_fixedNorm',q); % transform parameters from gaussian space to model space
...
fixed = 0.8;
free = {alpha beta f0 delta};
% -------------------------------------------------------------------------------------
% 2-4) Middle code is specific the the model
% -------------------------------------------------------------------------------------
if doprior == 1
    [fval,norm,V,ChoiceProb] = lik_UG1_etaf_fixedNorm(behavData.offer, behavData.choice,fixed,free,doprior,prior,q);
else
    [fval,norm,V,ChoiceProb] = lik_UG1_etaf_fixedNorm(behavData.offer, behavData.choice,fixed,free,doprior);
...
if dofit ==1
   ...
   fit         = struct;
   fit.xnames  = {'alpha'; 'beta';'f0';'delta'};
