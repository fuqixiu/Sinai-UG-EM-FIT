function [fval,fit] = mod_ms_UG1_etaf_adaptiveNorm(behavData,q, doprior,dofit,varargin)
...
[qt, bounds] = norm2par('ms_UG1_etaf_adaptiveNorm',q); % transform parameters from gaussian space to model space
...
% -------------------------------------------------------------------------------------
if doprior == 1
    [fval,norm,V,ChoiceProb] = lik_UG1_etaf_adaptiveNorm(behavData.offer, behavData.choice,fixed,free,doprior,prior,q);
else
    [fval,norm,V,ChoiceProb] = lik_UG1_etaf_adaptiveNorm(behavData.offer, behavData.choice,fixed,free,doprior);
