function [fval,fit] = mod_ms_UG0_FS_10(behavData,q, doprior,dofit,varargin)
% runs standard 0 Step UG Model
% Created MK Wittmann, Oct 2018
% Adapted BRK Shevlin, April 2023
% Adapted AD (written new!), July 2023
%
...
[qt, bounds] = norm2par('ms_UG0_FS_10',q); % transform parameters from gaussian space to model space
...
% -------------------------------------------------------------------------------------
if doprior == 1
    [fval,norm,V,ChoiceProb] = lik_UG0_FS_10(behavData.offer, behavData.choice,fixed,free,doprior,prior,q);
else
    [fval,norm,V,ChoiceProb] = lik_UG0_FS_10(behavData.offer, behavData.choice,fixed,free,doprior);
