function [fval, norm, V, ChoiceProb] = lik_UG1_etaf_fixedNorm(offer, resp, fixed, free, doprior, varargin)
    mn = 1;  
   ...
   [alpha, beta, f0, delta] = deal(free{:});
    ...
    for i = 1:n       
        ...
        CV(i) =FS(alpha, offer(i), norm);
        FVa(i) = max(0, FS(alpha,  max(offer(i)-delta, mn), norm));
        FVr(i) = max(0, FS(alpha,  max(offer(i)+delta, mn), norm));
